...
const mockDatabase = vi.hoisted(() => ({
  createProduct: vi.fn(),
  deleteProduct: vi.fn(),
  ensureQuoteConversationForSupplier: vi.fn(),
...
describe('SupplierDashboard', () => {
  ...
  beforeEach(() => {
    ...
    mockDatabase.ensureQuoteConversationForSupplier.mockResolvedValue(seed.quoteConversations[0]);
  ...
  it('muestra summary y cartera buyer desde datos reales sin exponer planes', async () => {
  ...
  it('permite responder una RFQ abierta desde el inbox', async () => {
    ...
    await screen.findByLabelText(/Precio estimado/i);

    await user.type(screen.getByLabelText(/Precio estimado/i), '1320');
    await user.type(screen.getByLabelText(/Tiempo estimado de entrega/i), '48 horas');
    await user.type(screen.getByLabelText(/Mensaje al comprador/i), 'Incluye entrega en frio');
  ...
  it('permite responder una RFQ por chat sin precio obligatorio', async () => {
    const user = userEvent.setup();
    const openQuote = buildQuoteRequest({
      id: 'quote-open-chat',
      buyer_id: 'buyer-1',
    ...
    const quoteButtons = await screen.findAllByRole('button', { name: 'Cotizar' });
    await user.click(quoteButtons[0]);
    expect(await screen.findByLabelText(/Precio estimado/i)).not.toBeRequired();

    await user.type(screen.getByLabelText(/Mensaje al comprador/i), 'Lo reviso y te confirmo precio por volumen.');
    await user.click(screen.getByRole('button', { name: 'Enviar mensaje' }));

    await waitFor(() => {
      expect(mockDatabase.submitOffer).not.toHaveBeenCalled();
      expect(mockDatabase.ensureQuoteConversationForSupplier).toHaveBeenCalledWith({
        quoteId: 'quote-open-chat',
        buyerUserId: 'buyer-1',
        supplierUserId: 'supplier-1',
        startedByUserId: 'supplier-1',
      });
      expect(mockDatabase.sendQuoteConversationMessage).toHaveBeenCalledWith({
        conversationId: 'conversation-1',
        senderUserId: 'supplier-1',
        body: 'Lo reviso y te confirmo precio por volumen.',
      });
    });
  });
  ...
  it('aplica feature gates para plan starter cuando el limite mensual ya fue usado', async () => {
    ...
    expect(await screen.findByRole('button', { name: 'Limite operativo' })).toBeInTheDocument();
